...
En esta sección se repasarán las variables objetivo, las razones de su elección y su anatomía. A continuación, se detallará la construicción del conjunto de datos y se realizará su correspondiente análisis exploratorio. Finalmente, se cierra la sección mostrando los pasos del preprocesado que se han llevado a cabo, previo al entrenamiento de los modelos.


\section{La variable objetivo}
\label{cap:variable}
...
objetivo, un proceso que supuso grandes desafíos. Se comienza introduciendo el concepto de los
...
del cual una muestra deja de tener señal que aprender y se dedica un pocas páginas a describir
...
\subsubsection{Definición y elección}
...
\begin{figure}[htbp]
    ...
    \input{figuras/cap05/pauli_tikz}
...
\begin{table}[htbp]
    ...
    {\footnotesize
    Fuente: elaboración propia. \begin{tabular}{@{}llp{0.44\textwidth}@{}}
        \toprule
...
\subsubsection{Impacto del ruido a un valor esperado}
...
% =====================================================================================
\subsection{El factor de supervivencia}
\label{sec:factor}
% =====================================================================================

\subsubsection{Candidatas a variable objetivo y elección}
...
\begin{table}[htbp]
    ...
    {\footnotesize
    Fuente: elaboración propia. \begin{tabular}{@{}lcc@{}}
        \toprule
    ...
    {\footnotesize Umbral de señal $0{,}01$.}
\end{table}
...
\subsubsection{La reformulación}
...
\textit{Una aclaración}: para reconstruir la degradación, en gran parte de la literatura \citeA{placidi2026deep} recurren al valor ruidoso $\langle O \rangle_{\text{ruidoso}}$ (obtenido tras ejecución en \textbf{hardware cuántico real} y costoso pero acanzable) y del valor $\langle O \rangle_{\text{exacto}}$ (obtenido tras ejecución en \textbf{hardware clásico} y costoso pero con alcance limitado, explicado en el Capítulo~\ref{sec:hardware})
...
Conviene profundizar en la idea propuesta, ya que este planteamiento no rompe el \textbf{paradigma pre-ejecución}. Las razones son las siguientes:

\begin{itemize}
    \item \textbf{La entrada del modelo} está compuesta únicamente por el circuito y la telemetría del chip. El modelo \textbf{nunca ve} el resultado de la ejecución, ni al entrenar ni al predecir.
...
\begin{figure}[htbp]
    ...
    \input{figuras/cap05/reconstruccion_tikz}
...
\begin{table}[htbp]
    ...
    \label{tab:barrido}
    {\footnotesize
    Fuente: elaboración propia. \begin{tabular}{@{}lrrrrrr@{}}
        \toprule
    ...
    {\footnotesize Sin umbral hay $66{.}720$ etiquetas. En negrita,
    el umbral elegido.}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Limpia la mayor parte de los valores máximos altos}: los factores
          ...
          de $6{,}67$. Un factor negativo significa que el valor medido (el ruidoso) cambió de signo
          respecto al exacto.
...
% =====================================================================================
% 🔴 EL TERMINO AFIN — TRAIDO DESDE EL ANTIGUO ANEXO C (sep-2026, indicacion del tutor:
%    «meterlo en la memoria pero mas resumido»). El anexo se RETIRO; este es ahora el
%    unico sitio donde se desarrolla el modelo afin. No reponer el anexo.
%
% ⚠️ SE CONSERVA UNA SOLA ECUACION. El anexo tenia dos: la afin y el paso algebraico de
%    dividir por `e` para llegar a `f + b/e`. Ese segundo paso se cuenta en prosa a
%    proposito: es algebra elemental y, escrito, desvia el foco del apartado, que va de
%    elegir un umbral. Criterio del tutor.
%
% ⚠️ VA AQUI Y NO EN §\ref{sec:reformulacion} porque es aqui donde el lector acaba de
%    encontrarse los factores negativos. La explicacion al lado del fenomeno, sin saltos.
%
% Fuente de las cifras: doc/changes/2026-08_cambio_decisiones.md §1 y §3 (dataset v2,
% ajuste sobre una submuestra de 3.000 circuitos). Citables.
% =====================================================================================

El factor negativo merece una explicación. El factor de supervivencia asume que el ruido es
\textbf{multiplicativo puro}: que el valor medido es el exacto multiplicado por una fracción
que sobrevive. Medido, no lo es del todo, y el modelo real es \textbf{afín}:
...
\noindent El término $b$ es un desplazamiento que \textbf{no depende de cuánta señal había}, sino una constante que reciben las medidas y que son empujadas al signo opuesto.


\textbf{El término $b$ no se modela.} Se asume el factor multiplicativo puro por simplificar el
trabajo, y la consecuencia queda declarada: los circuitos en los que el factor cambia de signo
caen fuera de lo que esta variable objetivo puede representar.
...
\begin{table}[htbp]
    ...
    {\footnotesize Conjunto de entrenamiento.}
\end{table}
...
\subsubsection{El factor de supervivencia frente al tamaño}
\label{sec:f_tamano}
