...
\subsection{Razón de construirlo}
\label{sec:gemelo}
...
\subsection{El procesador y su calibración}
...
\begin{figure}[htbp]
    ...
    \input{figuras/cap06/tres_ejes_tikz}
...
cada bloque. La matriz que queda después está en el apartado~\ref{sec:prep}.

% Figura propia, de notebooks/01_eda/01g, copiada desde
% figures/eda/entradas/01_colinealidad.pdf.
% ⚠️ Esta es la de las 54 columnas. La version REDUCIDA, de 34, va en §\ref{sec:prep}:
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.88\textwidth]{cap06/colinealidad}
...
\begin{itemize}
    \item \textbf{Partición temporal}: días $0$--$24$ para entrenar, $25$--$29$ para validar y $30$--$41$ para probar.
    \item \textbf{Las filas}: \begin{itemize}
        \item \textbf{No se imputa nada.}
        \item \textbf{Los factores negativos se predicen, pero no se evalúan.} Son el $1{,}41$\,\%
          de \texttt{train}.
        \item \textbf{Los factores mayores que uno no se tocan.} Son el $7{,}85$\,\% de las
          etiquetas y son representativos.
        \item Los modelos tabulares ajustan \textbf{un modelo por observable} y eliminan las filas
          sin etiqueta de ese observable; los modelos de grafos son \textbf{multi-salida} y se
          queda con todas las etiquetas, tapando con una máscara las casillas que faltan.
    \end{itemize}
    
    \item \textbf{Rama tabular:} \begin{itemize}
        \item Se descartaron las 20 columnas seleccionadas redundantes (véase el resultado en Figura~\ref{fig:colinealidad_limpia})

% ⚠️ FORMATO: titulo arriba y fuente abajo (Instrucciones §1.3, p. 6).
% Figura propia, de notebooks/01_eda/01g, copiada desde
% figures/eda/entradas/02_colinealidad_limpia.pdf.
% ⚠️ Es la version REDUCIDA, de 34 columnas. La de las 54 esta en §\ref{sec:colinealidad}:
%    van separadas a proposito, para que se vea el antes y el despues del descarte.
\begin{figure}[htbp]
    \centering
    \caption{Correlación entre las $34$ variables que quedan tras el descarte.}
    \label{fig:colinealidad_limpia}

    \includegraphics[width=0.88\textwidth]{cap06/colinealidad_limpia}

    {\footnotesize Fuente: elaboración propia.}
\end{figure}

        \item Luego se aplicaron en este orden: una \textbf{transformación logarítmica} a todas las columnas con asimetría en modulo mayor que uno, y \textbf{escalado} (Nota: a Ranfom Rorest se aplicaron las mismas transformaciones, aunque no sirvieran, para entrenar en ambos modelo con el mismo conjunto de datos). 
    \end{itemize}
    \item \textbf{Rama de grafo:} \begin{itemize}
        \item El vector nodo pasa de 25 dimensiones a 21.
        \item La \textbf{poda de 5 dimensiones}, reflejado el motivo en la Tabla~\ref{tab:poda}.
        \item \textbf{Los ceros:} En el $78{,}88$\,\% de los nodos las seis dimensiones del qubit objetivo valen cero porque no hay segundo qubit. Tras estandarizar valen distinto a cero e introduce información falsa. \textbf{Solución:} ajustar con los nodos de dos qubits y \textbf{después reimponer el cero exacto}. 
        \item \textbf{El ángulo se sustituye por dos columnas:} seno y su coseno.
        \item \textbf{Las constantes se congelan.} El resto se estandariza ajustando solo con el conjunto de entrenamiento,
        y las constantes \textbf{se leen de un fichero}.
    \end{itemize}
\end{itemize}
...
El preprocesado tabular y el del grafo son \textbf{independientes}. Comparten la partición y el umbral, pero no comparten el tratamiento.
...
a no mitigar}, y hacerlo \textbf{fuera de la distribución de entrenamiento}. 
...
Además, para medir la fidelidad del valor mitigado se emplearán MAE y la `mejora relativa'.

\begin{equation}
\label{eq:metricas}
R^2 = 1 - \frac{\sum_i (f_i - \hat f_i)^2}{\sum_i (f_i - \bar f)^2} ,
\qquad
\text{MAE} = \frac{1}{n}\sum_i \left| f_i - \hat f_i \right| ,
\end{equation}
...
generalización} (tipo, tamaño y tiempo) descritos en el apartado~\ref{sec:composicion}. 
